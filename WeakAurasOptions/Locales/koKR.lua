...
L["Custom trigger event tooltip"] = [=[트리거를 확인할 이벤트를 작성하십시오. 여러 이벤트는 쉼표 또는 공백으로 구분되어야 합니다.

|cFF4444FF예:|r
UNIT_POWER  UNIT_AURA, PLAYER_TARGET_CHANGED]=]
L["Custom trigger status tooltip"] = [=[트리거를 확인할 이벤트를 작성하십시오. 여러 이벤트는 쉼표 또는 공백으로 구분되어야 합니다.
이것은 상태 트리거이므로, 지정된 이벤트는 예상 인수 없이 WeakAuras에 의해 전달될 수 있습니다.
...
이 텍스트를 동적으로 만들기 위해 사용할 수 있는 여러 가지 특수 코드가 있습니다:

|cFFFFCC00%p|r - 진행 상황 - 타이머의 남은 시간 또는 비타이머 값
|cFFFFCC00%t|r - 전체 - 타이머의 최대 시간 또는 최대 비타이머 값
|cFFFFCC00%n|r - 이름 - 표시 이름(주로 오라 이름) 또는 동적 이름이 없는 경우 표시의 ID
|cFFFFCC00%i|r - 아이콘 - 표시와 관련된 아이콘
|cFFFFCC00%s|r - 중첩 - 오라의 중첩 수(주로)
|cFFFFCC00%c|r - 사용자 정의 - 문자열 값을 반환하는 사용자 정의 Lua 함수를 정의할 수 있습니다. %c1은 첫 번째 반환 값으로, %c2는 두 번째 반환 값으로 대체됩니다.
|cFFFFCC00%%|r - % - 퍼센트 기호를 표시

기본적으로 이러한 코드는 동적 정보에서 선택한 트리거의 정보를 표시합니다. 특정 트리거의 정보는 예를 들어 %2.p와 같이 표시할 수 있습니다.
]=]
