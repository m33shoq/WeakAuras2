if not M33kAuras.IsLibsOK() then
  return
end
---@type string
local AddonName = ...
...
M33kAurasProfilingReportMixin = {}
function M33kAurasProfilingReportMixin:OnShow()
  ...
  self:SetTitle(L["M33kAuras Profiling Report"])
  self:SetSize(500, 300)
end

function M33kAurasProfilingReportMixin:ClearText()
  self.ScrollBox.messageFrame:SetText("")
end

function M33kAurasProfilingReportMixin:AddText(v)
  if not v then
    return
  end
  --- @type string?
...
local RegisterProfile = function(startType)
  ...
  if startType == "encounter" then
    M33kAurasProfilingFrame:UnregisterAllEvents()
    prettyPrint(L["Your next encounter will automatically be profiled."])
    M33kAurasProfilingFrame:RegisterEvent("ENCOUNTER_START")
    M33kAurasProfilingFrame:RegisterEvent("ENCOUNTER_END")
  ...
  elseif startType == "combat" then
    ...
    M33kAurasProfilingFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    M33kAurasProfilingFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    currentProfileState = startType
    delayedStart = true
  ...
  elseif startType and startType:match("%d") then
    ...
    ProfilingTimer = M33kAuras.timer:ScheduleTimer(M33kAuras.StopProfile, time)
    currentProfileState = "profiling"
  else
    M33kAurasProfilingFrame:UnregisterAllEvents()
  ...
  M33kAurasProfilingFrame:UpdateButtons()
  return delayedStart
end

---@diagnostic disable-next-line: duplicate-set-field
function M33kAuras.StartProfile(startType)
...
function M33kAuras.StopProfile()
  ...
  if M33kAurasProfilingFrame then
    M33kAurasProfilingFrame:UnregisterAllEvents()
    M33kAurasProfilingFrame:UpdateButtons()
  end

  if ProfilingTimer then
    M33kAuras.timer:CancelTimer(ProfilingTimer)
    ProfilingTimer = nil
  end
end

function M33kAuras.ToggleProfile()
  if not profileData.systems.time or profileData.systems.time.count ~= 1 then
    M33kAuras.StartProfile()
  else
...
local function ColoredSpike(spike)
  local r, g, b
  if spike < 2 then
    r, g, b = M33kAuras.GetHSVTransition(spike / 2, 0, 1, 0, 1, 1, 1, 0, 1)
  elseif spike < 2.5 then
    r, g, b = M33kAuras.GetHSVTransition((spike - 2) * 2, 1, 1, 0, 1, 1, 0.65, 0, 1)
  elseif spike < 3 then
    r, g, b = M33kAuras.GetHSVTransition((spike - 2.5) * 2, 1, 0.65, 0, 1, 1, 0, 0, 1)
  else
    r, g, b = 1, 0, 0
  end
...
function M33kAuras.PrintProfile()
  local popup = M33kAurasProfilingReport
  ...
  if M33kAurasProfilingFrame and M33kAurasProfilingFrame:IsShown() then
    popup:SetParent(M33kAurasProfilingFrame)
    popup:SetPoint("TOPLEFT", M33kAurasProfilingFrame, "TOPRIGHT", 5, 0)
  else
    popup:SetParent(UIParent)
    if M33kAurasSaved.ProfilingWindow then
      popup:SetPoint("TOPLEFT", UIParent, "TOPLEFT", M33kAurasSaved.ProfilingWindow.xOffset or 0, M33kAurasSaved.ProfilingWindow.yOffset or 0)
    else
...
function M33kAurasProfilingMixin:OnShow()
  ...
  ButtonFrameTemplate_HidePortrait(self)
  self:SetTitle(L["M33kAuras Profiling"])
  ...
  view:SetElementInitializer("M33kAurasProfilingLineTemplate", function(frame, elementData)
...
function M33kAurasProfilingStopButton_OnClick(self)
  if currentProfileState == "profiling" then
    M33kAuras.StopProfile()
  else
...
function M33kAurasProfilingMixin:InitDropDown()
...
function M33kAurasProfilingMixin:InitModeDropDown()
  local function Initializer(dropDown, level)
    for i = 1, 2 do
      local info = UIDropDownMenu_CreateInfo()
      info.text = modes[i]
...
local lastRefresh
function M33kAurasProfilingMixin:RefreshBars(_, force)
...
function M33kAurasProfilingMixin:ResetBars()
  self.bars:Flush()
end

function M33kAurasProfilingMixin:UpdateBar(name, time, pct, spike)
  ...
  if elementData then
    ...
    local button = M33kAurasProfilingFrame.ScrollBox:FindFrame(elementData)
    if button then
...
function M33kAurasProfilingMixin:SortByColumnIndex(index)
...
function M33kAurasProfilingMixin:UpdateButtons()
...
function M33kAurasProfilingMixin:Start()
  self:Show()
end

function M33kAurasProfilingMixin:Stop()
  M33kAuras.StopProfile()
  self:UpdateButtons()
  self:ResetBars()
end

function M33kAurasProfilingMixin:Toggle()
...
function M33kAurasProfilingColumnDisplay_OnClick(self, columnIndex)
