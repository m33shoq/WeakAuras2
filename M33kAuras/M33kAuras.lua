...
local ADDON_NAME = "M33kAuras"
---@class M33kAuras
local M33kAuras = M33kAuras
local L = M33kAuras.L
local versionString = M33kAuras.versionString
local prettyPrint = M33kAuras.prettyPrint

M33kAurasTimers = setmetatable({}, {__tostring=function() return "M33kAuras" end})
LibStub("AceTimer-3.0"):Embed(M33kAurasTimers)
...
function M33kAurasTimers:ScheduleTimerFixed(func, delay, ...)
  if (delay < Private.maxTimerDuration) then
    if delay + GetTime() > maxUpTime then
      M33kAuras.prettyPrint(M33kAuras.L["Can't schedule timer with %i, due to a World of Warcraft bug with high computer uptime. (Uptime: %i). Please restart your computer."]:format(delay, GetTime()))
      return
    end
...
if CustomNames then
  M33kAuras.GetName = CustomNames.Get
  M33kAuras.UnitName = CustomNames.UnitName
  M33kAuras.GetUnitName = CustomNames.GetUnitName
...
local timer = M33kAurasTimers
M33kAuras.timer = timer

local loginQueue = {}
local queueshowooc

function M33kAuras.InternalVersion()
...
do
  ...
  local function waErrorHandler(errorMessage)
    ...
    if currentErrorHandlerId then
      data = M33kAuras.GetData(currentErrorHandlerId)
    ...
    table.insert(juicedMessage, L["M33kAuras Version: %s"]:format(M33kAuras.versionString))
...
function Private.LoadOptions(msg)
  if not(C_AddOns.IsAddOnLoaded("M33kAurasOptions")) then
    if not M33kAuras.IsLoginFinished() then
      prettyPrint(Private.LoginMessage())
      loginQueue[#loginQueue + 1] = M33kAuras.OpenOptions
    elseif InCombatLockdown() then
      ...
      local loaded, reason = C_AddOns.LoadAddOn("M33kAurasOptions");
      if not(loaded) then
        reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
        M33kAuras.prettyPrint(string.format(L["Options could not be loaded, the addon is %s"], reason));
        return false;
      end
    end
  end
...
function M33kAuras.OpenOptions(msg)
  if Private.NeedToRepairDatabase() then
    StaticPopup_Show("M33kAuras_CONFIRM_REPAIR", nil, nil, {reason = "downgrade"})
  elseif (M33kAuras.IsLoginFinished() and Private.LoadOptions(msg)) then
    M33kAuras.ToggleOptions(msg, Private);
  end
end
...
function SlashCmdList.M33kAuras(input)
  local args, msg = {}, nil
  ...
  if msg == "pstart" then
    M33kAuras.StartProfile(args[1]);
  elseif msg == "pstop" then
    M33kAuras.StopProfile();
  elseif msg == "pprint" then
    M33kAuras.PrintProfile();
  elseif msg == "pcancel" then
    M33kAuras.CancelScheduledProfile()
  ...
  elseif msg == "repair" then
    StaticPopup_Show("M33kAuras_CONFIRM_REPAIR", nil, nil, {reason = "user"})
  ...
  elseif msg == "ff" or msg == "feat" or msg == "feature" then
    ...
    M33kAuras.OpenOptions(msg);
  end
end

if not M33kAuras.IsLibsOK() then return end

function M33kAuras.ToggleMinimap()
  M33kAurasSaved.minimap.hide = not M33kAurasSaved.minimap.hide
  if M33kAurasSaved.minimap.hide then
    LDBIcon:Hide("M33kAuras");
    prettyPrint(L["Use /wa minimap to show the minimap icon again."])
  else
    LDBIcon:Show("M33kAuras");
  end
end

BINDING_HEADER_M33kAuras = ADDON_NAME
BINDING_NAME_M33kAurasTOGGLE = L["Toggle Options Window"]
BINDING_NAME_M33kAurasPROFILINGTOGGLE = L["Toggle Performance Profiling Window"]
BINDING_NAME_M33kAurasPRINTPROFILING = L["Print Profiling Results"]

-- An alias for M33kAurasSaved, the SavedVariables
-- Noteable properties:
--  debug: If set to true, WeakAura.debug() outputs messages to the chat frame
--  displays: All aura settings, keyed on their id

---@class M33kAurasSaved
local db;

-- While true no events are handled. E.g. M33kAuras is paused while the Options dialog is open
local paused = true;
...
M33kAuras.raidUnits = {};
M33kAuras.raidpetUnits = {};
M33kAuras.partyUnits = {};
M33kAuras.partypetUnits = {};
M33kAuras.petUnitToUnit = {
  pet = "player"
}
M33kAuras.unitToPetUnit = {
  player = "pet"
}
do
  for i=1,40 do
    M33kAuras.raidUnits[i] = "raid"..i
    M33kAuras.raidpetUnits[i] = "raidpet"..i
    M33kAuras.petUnitToUnit["raidpet"..i] = "raid"..i
    M33kAuras.unitToPetUnit["raid"..i] = "raidpet"..i
  end
  for i=1,4 do
    M33kAuras.partyUnits[i] = "party"..i
    M33kAuras.partypetUnits[i] = "partypet"..i
    M33kAuras.petUnitToUnit["partypet"..i] = "party"..i
    M33kAuras.unitToPetUnit["party"..i] = "partypet"..i
  end
end

---@param unit UnitToken
---@return boolean isPet
M33kAuras.UnitIsPet = function(unit)
  return M33kAuras.petUnitToUnit[unit] ~= nil
end
...
function M33kAuras.RegisterSubRegionType(name, displayName, supportFunction, createFunction, modifyFunction, onAcquire, onRelease, default, addDefaultsForNewAura, properties, supportsAdd)
  if not(name) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - name is not a string", 2);
  elseif not(displayName) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - display name is not defined".." "..name, 2);
  elseif(type(displayName) ~= "string") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - display name is not a string", 2);
  elseif not(supportFunction) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - support function is not defined", 2);
  elseif(type(supportFunction) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - support function is not a function", 2);
  elseif not(createFunction) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - creation function is not a function", 2);
  elseif not(modifyFunction) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - modification function is not defined", 2);
  elseif(type(modifyFunction) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - modification function is not a function", 2)
  elseif not(onAcquire) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - onAcquire function is not defined", 2);
  elseif(type(onAcquire) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - onAcquire function is not a function", 2)
  elseif not(onRelease) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - onRelease function is not defined", 2);
  elseif(type(onRelease) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - onRelease function is not a function", 2)
  elseif not(default) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - default options are not defined", 2);
  elseif(type(default) ~= "table" and type(default) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - default options are not a table or a function", 2);
  elseif(addDefaultsForNewAura and type(addDefaultsForNewAura) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - addDefaultsForNewAura function is not nil or a function", 2)
  elseif(subRegionTypes[name]) then
    error("Improper arguments to M33kAuras.RegisterSubRegionType - region type \""..name.."\" already defined", 2);
  else
...
function M33kAuras.RegisterSubRegionOptions(name, createFunction, description, getAnchors)
  if not(name) then
    error("Improper arguments to M33kAuras.RegisterSubRegionOptions - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to M33kAuras.RegisterSubRegionOptions - name is not a string", 2);
  elseif not(createFunction) then
    error("Improper arguments to M33kAuras.RegisterSubRegionOptions - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionOptions - creation function is not a function", 2);
  elseif(getAnchors and type(getAnchors) ~= "function") then
    error("Improper arguments to M33kAuras.RegisterSubRegionOptions - getAnchors function is not a function", 2);
  elseif(subRegionOptions[name]) then
    error("Improper arguments to M33kAuras.RegisterSubRegionOptions - region type \""..name.."\" already defined", 2);
  else
...
---@diagnostic disable-next-line: duplicate-set-field (it's replaced in M33kAurasOptions.lua)
...
function M33kAuras.GetActiveConditions(id, cloneId)
  ...
  if (data.actions) then
    if data.actions.init then
      if data.actions.init.do_custom and data.actions.init.custom then
        local func = M33kAuras.LoadFunction("return function() "..(data.actions.init.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["init"] = func
      end
      if data.actions.init.do_custom_load and data.actions.init.customOnLoad then
        local func = M33kAuras.LoadFunction("return function() "..(data.actions.init.customOnLoad).."\n end", data.id);
      ...
      if data.actions.init.do_custom_unload and data.actions.init.customOnUnload then
        local func = M33kAuras.LoadFunction("return function() "..(data.actions.init.customOnUnload).."\n end", data.id);
    ...
    if (data.actions.start) then
      if (data.actions.start.do_custom and data.actions.start.custom) then
        local func = M33kAuras.LoadFunction("return function() "..(data.actions.start.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["start"] = func;
      end

      if (data.actions.start.do_message and data.actions.start.message_custom) then
        local func = M33kAuras.LoadFunction("return "..(data.actions.start.message_custom), data.id);
    ...
    if (data.actions.finish) then
      if (data.actions.finish.do_custom and data.actions.finish.custom) then
        local func = M33kAuras.LoadFunction("return function() "..(data.actions.finish.custom).."\n end", data.id);
      ...
      if (data.actions.finish.do_message and data.actions.finish.message_custom) then
        local func = M33kAuras.LoadFunction("return "..(data.actions.finish.message_custom), data.id);
  ...
  elseif M33kAuras.IsMists() then
    ...
    for tier = 1, MAX_TALENT_TIERS do
      for column = 1, NUM_TALENT_COLUMNS do
        ...
        -- Get the icon and name from the talent cache and record it in the table that will be used by M33kAurasOptions
        if (talentName and talentIcon) then
...
-- when using this function, do not name your global data table "M33kAurasCompanion"
function M33kAuras.AddCompanionData(data)
  M33kAuras.DeepMixin(Private.CompanionData, data)
end

-- add data from versions of companion compatible addon that does not use M33kAuras.AddCompanionData yet
local function AddLegacyCompanionData()
  local CompanionData = M33kAurasCompanion and M33kAurasCompanion.M33kAuras or M33kAurasCompanion
  if CompanionData then
    M33kAuras.AddCompanionData(CompanionData)
  end
end
...
function Private.PostAddCompanion()
  ...
  if count and count > 0 then
    M33kAuras.prettyPrint(L["There are %i updates to your auras ready to be installed!"]:format(count))
  end
  -- nag if new installs
  if Private.CompanionData.stash and next(Private.CompanionData.stash) then
    M33kAuras.prettyPrint(L["You have new auras ready to be installed!"])
  end
end
...
local function tooltip_draw(isAddonCompartment, blizzardTooltip)
  ...
  tooltip:ClearLines()
  tooltip:AddDoubleLine("M33kAuras", versionString)
...
M33kAuras.GenerateTooltip = tooltip_draw;
...
local Broker_M33kAuras;
Broker_M33kAuras = LDB:NewDataObject("M33kAuras", {
  type = "launcher",
  text = "M33kAuras",
  ...
  OnEnter = function(self)
    colorFrame:SetScript("OnUpdate", function(self, elaps)
      ...
      Broker_M33kAuras.iconR = r + (r2 - r) * colorElapsed / colorDelay;
      Broker_M33kAuras.iconG = g + (g2 - g) * colorElapsed / colorDelay;
      Broker_M33kAuras.iconB = b + (b2 - b) * colorElapsed / colorDelay;
    end);
    local elapsed = 0;
    local delay = 1;
...
do -- Archive stuff
  ...
  local function OpenArchive()
    if Archivist:IsInitialized() then
      ...
      if not C_AddOns.IsAddOnLoaded("M33kAurasArchive") then
        local ok, reason = C_AddOns.LoadAddOn("M33kAurasArchive")
        if not ok then
          reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
          error(string.format(L["Could not load M33kAuras Archive, the addon is %s"], reason))
        end
      end
      if type(M33kAurasArchive) ~= "table" then
        M33kAurasArchive = {}
      end
      Archivist:Initialize(M33kAurasArchive)
    end
    return Archivist
  end

  function M33kAuras.LoadFromArchive(storeType, storeID)
...
function M33kAuras.IsLoginFinished()
    ...
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        ...
        if (guid and db.CurrentEncounter.boss_guids [guid]) then
          ...
          M33kAuras.CurrentEncounter = db.CurrentEncounter
          return true
        end
      end
    end
...
function Private.Login(takeNewSnapshots)
  local loginFunc = (function() -- coroutine.create
    ...
    coroutine.yield(100)
    if db.history then
      local histRepo = M33kAuras.LoadFromArchive("Repository", "history")
      local migrationRepo = M33kAuras.LoadFromArchive("Repository", "migration")
      ...
      -- history is now in archive so we can shrink M33kAurasSaved
      db.history = nil
    end
    ...
    for id, data in pairs(db.displays) do
      if(id ~= data.id) then
        print("|cFF8800FFM33kAuras|r detected a corrupt entry in M33kAuras saved displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
...
Private.frames["M33kAuras Main Frame"] = M33kAurasFrame;
M33kAurasFrame:SetAllPoints(UIParent);
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if(event == "ADDON_LOADED") then
    if(... == ADDON_NAME) then
      ...
      -- if not db.disableRuLocale and M33kAuras.LoadLocale_ruRU then
      --   M33kAuras.LoadLocale_ruRU()
      -- end
      ...
      LDBIcon:Register("M33kAuras", Broker_M33kAuras, db.minimap);
    end
...
function M33kAuras.IsImporting()
  return importing;
end

function M33kAuras.IsPaused()
  return paused;
end
...
function M33kAuras.InLoadingScreen()
...
function Private.ResumeAllDynamicGroups(suspended)
  for _, id in ipairs(suspended) do
    local region = M33kAuras.GetRegion(id)
...
local function StoreBossGUIDs()
  ...
  if (M33kAuras.CurrentEncounter and M33kAuras.CurrentEncounter.boss_guids) then
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        local guid = UnitGUID ("boss" .. i)
        if (guid) then
          M33kAuras.CurrentEncounter.boss_guids [guid] = true
        end
      end
    end
    db.CurrentEncounter = M33kAuras.CurrentEncounter
  end
  Private.StopProfileSystem("boss_guids")
end

local function DestroyEncounterTable()
  if (M33kAuras.CurrentEncounter) then
    wipe(M33kAuras.CurrentEncounter)
  end
  M33kAuras.CurrentEncounter = nil
  db.CurrentEncounter = nil
end
...
local function CreateEncounterTable(encounter_id)
  ...
  return M33kAuras.CurrentEncounter
end
...
function M33kAuras.InstanceType()
  return (GetInstanceTypeAndSize())
end

---@return string difficulty
function M33kAuras.InstanceDifficulty()
  return select(2, GetInstanceTypeAndSize())
end

---@return number? difficultyID
function M33kAuras.InstanceTypeRaw()
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  local encounter_id = M33kAuras.CurrentEncounter and M33kAuras.CurrentEncounter.id or 0
  ...
  if (M33kAuras.CurrentEncounter) then
    if (instanceId ~= M33kAuras.CurrentEncounter.zone_id and not inCombat) then
      encounter_id = 0
      DestroyEncounterTable()
    end
  end
  ...
  local addonRestrictionsActive = M33kAuras.IsSecretStateActive()
  ...
  for id in pairs(toCheck) do
    ...
    if (data and not data.controlledChildren) then
      ...
      -- elseif M33kAuras.IsWrathClassic() then
...
function Private.LoadDisplays(toLoad, ...)
  for id in pairs(toLoad) do
    ...
    if Private.DebugLog.IsEnabled(uid) then
      M33kAuras.prettyPrint(L["Debug Logging enabled for '%s'"]:format(id))
...
function Private.GetDataByUID(uid)
  return M33kAuras.GetData(UIDtoID[uid])
end
...
---@private
function M33kAuras.Delete(data)
...
function M33kAuras.Rename(data, newid)
  ...
  if(data.parent) then
    ...
    local parentRegion = M33kAuras.GetRegion(data.parent)
...
function M33kAuras.DeepMixin(dest, source)
...
function Private.NeedToRepairDatabase()
  return db.dbVersion and db.dbVersion > M33kAuras.InternalVersion()
end
...
local function RepairDatabase()
  local func = (function()
    ...
    db.dbVersion = M33kAuras.InternalVersion()
...
StaticPopupDialogs["M33kAuras_CONFIRM_REPAIR"] = {
  text = "",
  button1 = L["Repair"],
  button2 = L["Cancel"],
  ...
  for _, data in pairs(db.displays) do
    if type(data.uid) == "string" then
      if seenUIDs[data.uid] then
        ...
        data.uid = M33kAuras.GenerateUniqueID()
...
function Private.CheckForAnchorCycle(source)
  ...
  while source do
    cycle[source] = true
    local data = M33kAuras.GetData(source)
    local target
    if data then
...
function Private.AddMany(tbl, takeSnapshots)
  ...
  for _, source in pairs(anchorTargets) do
    ...
    if Private.CheckForAnchorCycle(source) then
      M33kAuras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(source))
  ...
  if takeSnapshots then
    ...
    if #order > 0 then
      Private:Async({name = "snapshot"}, function()
        prettyPrint(L["M33kAuras is creating a rollback snapshot of your auras. This snapshot will allow you to revert to the current state of your auras if something goes wrong. This process may cause your framerate to drop until it is complete."])
  ...
  for _, data in ipairs(order) do
    if data.parent and bads[data.parent] then
      ...
      local ok = xpcall(M33kAuras.PreAdd, Private.GetErrorHandlerUid(data.uid, "PreAdd"), data, oldSnapshot)
      if not ok then
        prettyPrint(L["Unable to modernize aura '%s'. This is probably due to corrupt data or a bad migration, please report this to m33shoq."]:format(data.id))
    ...
    if not customOptionIsValid(option) or authorOptionKeys[option.key] then
      ...
      if optionClass == "simple" then
        if not option.key then
          option.key = M33kAuras.GenerateUniqueID()
        end
...
function Private.ClearSounds(uid, severity)
  ...
  M33kAuras.ClearAndUpdateOptions(data.id, true)
  M33kAuras.FillOptions()
end

function M33kAuras.PreAdd(data, snapshot)
    ...
    for _, subRegionData in ipairs(data.subRegions) do
      ...
      if subType and Private.subRegionTypes[subType] then
        if Private.subRegionTypes[subType].supports(data.regionType) then
          ...
          M33kAuras.prettyPrint(L["ERROR in '%s' unknown or incompatible sub element type '%s'"]:format(data.id, subType))
        end
      end
    end
  end
...
local function cycleCheck(data)
  ...
  if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame and data.anchorFrameFrame:sub(1, 10) == "WeakAuras:" then
    if Private.CheckForAnchorCycle(id) then
      M33kAuras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(id))
...
function pAdd(data, simpleChange)
  local id = data.id;
  if not(id) then
    error("Improper arguments to M33kAuras.Add - id not defined");
    return;
  end

  data.uid = data.uid or M33kAuras.GenerateUniqueID()
  if db.displays[id] and db.displays[id].uid ~= data.uid then
    print("Improper? arguments to M33kAuras.Add - id", id, "is assigned to a different uid.", data.uid, db.displays[id].uid)
  end
  if UIDtoID[data.uid] and UIDtoID[data.uid] ~= id then
    print("Improper? arguments to M33kAuras.Add - uid is assigned to a id. Uid:", data.uid, "assigned too:", UIDtoID[data.uid], "assigning now to", data.id)
  end
  ...
  if simpleChange then
    db.displays[id] = data
    cycleCheck(data)
    if M33kAuras.GetRegion(data.id) then
    ...
    if Private.IsGroupType(data) then
      ...
      if data.triggers.disjunctive == "custom" then
        triggerLogicFunc = M33kAuras.LoadFunction("return "..(data.triggers.customTriggerLogic or ""), data.id);
      end
...
function M33kAuras.Add(data, simpleChange)
  Private.TimeMachine:DestroyTheUniverse(data.id)
...
function Private.SetRegion(data, cloneId)
  ...
  if not(regionType) then
    ...
    if(not regionTypes[regionType]) then
      regionType = "fallback";
      print("Improper arguments to M33kAuras.CreateRegion - regionType \""..data.regionType.."\" is not supported in ".. data.id)
    end
    ...
    if not(id) then
      ...
      if(cloneId) then
        ...
        if (not region or region.regionType ~= data.regionType) then
          ...
          if(clonePool[data.regionType] and clonePool[data.regionType][1]) then
            ...
            local clone = regionTypes[data.regionType].create(M33kAurasFrame, data);
        ...
        if((not Private.regions[id]) or (not Private.regions[id].region) or Private.regions[id].regionType ~= regionType) then
          region = regionTypes[regionType].create(M33kAurasFrame, data);
          Private.regions[id] = {
      ...
      local loginFinished = M33kAuras.IsLoginFinished();
...
local function EnsureRegion(id)
  if not Private.regions[id] or not Private.regions[id].region then
    ...
    while(id) do
      ...
      if M33kAuras.GetRegion(id) then
        break
      end
    end

    for _, toCreateId in ipairs_reverse(aurasToCreate) do
      local data = M33kAuras.GetData(toCreateId)
      Private.SetRegion(data)
    end
  end
...
---@return table|nil
function M33kAuras.GetRegion(id, cloneId)
  if(cloneId and cloneId ~= "") then
...
function Private.SetAllStatesHidden(id, triggernum)
  local triggerState = M33kAuras.GetTriggerStateForTrigger(id, triggernum);
  local changed = false
...
function Private.ReleaseClone(id, cloneId, regionType)
  ...
  if region:IsProtected() then
    M33kAuras.prettyPrint(L["Error '%s' created a secure clone. We advise deleting the aura. For more information:\nhttps://github.com/m33shoq/M33kAuras/wiki/Protected-Frames"]:format(id))
  else
...
do
  local function frame_monitor_callback(event, frame, unit, previousUnit)
      ...
      for region, data in pairs(glow_frame_monitor) do
        ...
        then
          if not new_frame then
            new_frame = M33kAuras.GetUnitFrame(unit)
          end
    ...
    if type(anchor_unitframe_monitor) == "table" then
      for region, data in pairs(anchor_unitframe_monitor) do
        ...
        then
          if not new_frame then
            new_frame = M33kAuras.GetUnitFrame(unit) or M33kAuras.HiddenFrames
          end
          if new_frame ~= data.frame then
  ...
  LGF.RegisterCallback("M33kAuras", "FRAME_UNIT_UPDATE", frame_monitor_callback)
  LGF.RegisterCallback("M33kAuras", "FRAME_UNIT_ADDED", frame_monitor_callback)
  LGF.RegisterCallback("M33kAuras", "FRAME_UNIT_REMOVED", frame_monitor_callback)
end
  ...
  then
    ...
    if actions.glow_frame_type == "FRAMESELECTOR" then
      if actions.glow_frame:sub(1, 10) == "WeakAuras:" then
        ...
        if M33kAuras.GetData(frame_name) then
    ...
    elseif actions.glow_frame_type == "UNITFRAME" and region.state.unit then
      glow_frame = M33kAuras.GetUnitFrame(region.state.unit)
      should_glow_frame = true
    elseif actions.glow_frame_type == "NAMEPLATE" and region.state.unit then
      glow_frame = M33kAuras.GetUnitNameplate(region.state.unit)
      should_glow_frame = true
...
function Private.PerformActions(data, when, region)
  if (paused or M33kAuras.IsOptionsOpen()) then
    return;
  end;
  local actions;
  local formatters
...
-- This has to be in M33kAuras for now, because GetNameAndIcon can be called from the options
-- before the Options has access to Private
M33kAuras.GetNameAndIcon = wrapTriggerSystemFunction("GetNameAndIcon", "nameAndIcon");
...
do
  local hiddenTooltip;
  function M33kAuras.GetHiddenTooltip()
    if not(hiddenTooltip) then
      hiddenTooltip = CreateFrame("GameTooltip", "M33kAurasTooltip", nil, "GameTooltipTemplate");
...
function M33kAuras.GetAuraInstanceTooltipInfo(unit, auraInstanceId, filter)
  if M33kAuras.IsRetail() then
    local tooltipText = ""
    local tooltipData
...
function M33kAuras.GetAuraTooltipInfo(unit, index, filter)
  local tooltipText = ""
  if M33kAuras.IsRetail() then
    ...
    local tooltip = M33kAuras.GetHiddenTooltip();
    tooltip:ClearLines();
...
local FrameTimes = {};
function M33kAuras.ProfileFrames(all)
  UpdateAddOnCPUUsage();
...
local DisplayTimes = {};
function M33kAuras.ProfileDisplays(all)
  UpdateAddOnCPUUsage();
...
function M33kAuras.EnsureString(input)
  if (input == nil) then
    return "";
  end
...
function M33kAuras.RegisterTriggerSystem(types, triggerSystem)
...
function M33kAuras.RegisterTriggerSystemOptions(types, func)
  for _, v in ipairs(types) do
    Private.triggerTypesOptions[v] = func;
  end
end

function M33kAuras.GetTriggerStateForTrigger(id, triggernum)
...
function M33kAuras.GetActiveStates(id)
  return triggerState[id].activeStates
end

function M33kAuras.GetActiveTriggers(id)
...
do
  ...
  function Private.UpdateFakeStatesFor(id)
    if (M33kAuras.IsOptionsOpen() and visibleFakeStates[id]) then
      local data = M33kAuras.GetData(id)
      if (data) then
        ...
        Private.UpdatedTriggerState(id)
        if M33kAuras.GetMoverSizerId() == id then
...
local function startStopTimers(id, cloneId, triggernum, state)
  ...
  if type(state.autoHide) == "boolean" then
    if state.paused then
      ...
      if (state.expirationTime == nil or issecretvalue(state.expirationTime)) and type(state.duration) == "number" and not issecretvalue(state.duration) then
...
function Private.UpdatedTriggerState(id)
  ...
  local activeTriggerState = M33kAuras.GetTriggerStateForTrigger(id, newActiveTrigger);
...
local function ReplaceValuePlaceHolders(textStr, region, customCache, state, formatter, trigger)
  ...
  if customIndexSubStr then
    ...
    if custom then
      value = M33kAuras.EnsureString(value)
    end
  else
...
function M33kAuras.IsAuraActive(id)
  local active = triggerState[id]
...
local function ensureMouseFrame()
  ...
  ---@class Frame
  mouseFrame = CreateFrame("Frame", "M33kAurasAttachToMouseFrame", UIParent);
  mouseFrame.attachedVisibleFrames = {};
  mouseFrame:SetWidth(1);
  mouseFrame:SetHeight(1);

  local moverFrame = CreateFrame("Frame", "M33kAurasMousePointerFrame", mouseFrame);
  ...
  label:SetText("M33kAuras Anchor");
...
function Private.ensurePRDFrame()
  ...
  personalRessourceDisplayFrame = CreateFrame("Frame", "M33kAurasAttachToPRD", UIParent);
  ...
  local moverFrame = CreateFrame("Frame", "M33kAurasPRDMoverFrame", personalRessourceDisplayFrame);
  ...
  texture:SetAllPoints(moverFrame);
  texture:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\PRDFrame");
  ...
  personalRessourceDisplayFrame.OptionsOpened = function()
    ...
    if (KuiNameplatesCore and KuiNameplatesCore.profile) then
      ...
      personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\PRDFrameKui");
    else
      ...
      -- personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\PRDFrame");
    end
...
local function tryAnchorAgain()
  ...
  for id, _ in pairs(delayed) do
    ...
    if (data and region) then
      ...
      local parentData
      if data.parent then
        parentData = M33kAuras.GetData(data.parent)
...
local HiddenFrames = CreateFrame("Frame", "M33kAurasHiddenFrames")
HiddenFrames:Hide()
M33kAuras.HiddenFrames = HiddenFrames
...
local function GetAnchorFrame(data, region, parent)
  ...
  if (anchorFrameType == "NAMEPLATE") then
    ...
    if unit then
      local frame = unit and M33kAuras.GetUnitNameplate(unit)
  ...
  if (anchorFrameType == "UNITFRAME") then
    local unit = region.state and region.state.unit
    if unit then
      local frame = M33kAuras.GetUnitFrame(unit) or M33kAuras.HiddenFrames
      if frame then
  ...
  if (anchorFrameType == "CUSTOM" and region.customAnchorFunc) then
    ...
    if ok and frame then
      return frame
    elseif M33kAuras.IsOptionsOpen() then
      return parent
    else
      return HiddenFrames
    end
...
function Private.AnchorFrame(data, region, parent, force)
  ...
  and not M33kAuras.IsLoginFinished()
  and not force
  then
    ...
    if (data.anchorFrameParent or data.anchorFrameParent == nil
      ...
      region:SetParent(parent or M33kAurasFrame);
    end
...
function M33kAuras.SetModel(frame, unused, model_fileId, isUnit, isDisplayInfo)
...
function M33kAuras.SafeToNumber(input)
...
do
  ...
  function M33kAuras.IsUntrackableSoftTarget(unit)
  ...
  ---@param unit UnitToken
  ---@return boolean result
  function M33kAuras.UntrackableUnit(unit)
...
do
  ...
  ---@return string name
  ---@return string realm
  function M33kAuras.UnitNameWithRealm(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm = UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end

  function M33kAuras.UnitNameWithRealmCustomName(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm =  M33kAuras.UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end
end
...
function Private.ExecEnv.ParseBossModCheck(input)
  if not input then return end
  -- input is a string "1234, 4567, 8901" where each number is a boss mod encounter ID
  -- check if it is enabled with M33kAuras.IsBossModEnabled(encounterId)
  ...
  local matcher = {
    ...
    AddEncounterId = function(self, encounterId)
      ...
      if id then
        self.encounterIds[id] = true
      end
    end,
  }

  for encounterId in input:gmatch("%d+") do
    matcher:AddEncounterId(encounterId)
  end
  return matcher
end

function M33kAuras.IsAuraLoaded(id)
...
-- are categorized
---@private
function M33kAuras.GetTriggerCategoryFor(triggerType)
...
function Private.SortOrderForValues(values)
  ...
  table.sort(sortOrder, function(aKey, bKey)
    ...
    if type(aValue) == "string" and aValue:sub(1, #M33kAuras.newFeatureString) == M33kAuras.newFeatureString then
      aValue = aValue:sub(#M33kAuras.newFeatureString + 1)
    end

    if type(bValue) == "string" and bValue:sub(1, #M33kAuras.newFeatureString) == M33kAuras.newFeatureString then
      bValue = bValue:sub(#M33kAuras.newFeatureString + 1)
    end
  ...
  local function Traverse(data, includeSelf, includeGroups, includeLeafs)
    ...
    if data.controlledChildren then
      for _, child in ipairs(data.controlledChildren) do
        Traverse(M33kAuras.GetData(child), true, includeGroups, includeLeafs)
      end
    end
  end
